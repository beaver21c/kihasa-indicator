...
function resolveDataUrl(viewMode, selectedSido) {
  if (viewMode === 'sgg') return 'data/map/sgg.topojson';
  if (viewMode === 'sido_emd' && selectedSido) return `data/map/emd/${selectedSido}.topojson`;
  if (viewMode === 'sgg_emd' && selectedSido) return `data/map/emd/${selectedSido}.topojson`;
  return null;
}
...
export default function MapView() {
  ...
  // react-leaflet의 GeoJSON은 data 변경을 반영하지 않으므로 key로 강제 재생성한다.
  // 경계 데이터는 비동기로 도착하므로 피처 수까지 key에 포함해야 이전 경계가 남지 않는다.
  const dataKey = `${filtered?.features?.length ?? 0}`;
